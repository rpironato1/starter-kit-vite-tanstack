...
function MetricCard({ icon, title, value, subText, iconColorClass }: MetricCardProps) {
  return (
...
export function TokenUsageModal({ isOpen, onClose, usage }: TokenUsageModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <Backdrop onClick={onClose} className="z-[100]" />
          <motion.div
            ...
            <div className="relative bg-bg-modal border border-border w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] pointer-events-auto">
              ...
              <div className="flex items-center justify-between p-5 border-b border-border bg-bg-sidebar">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-text-primary">Token Usage Report</h2>
                    <p className="text-xs text-text-secondary">Detailed execution audit</p>
                  </div>
                </div>
                <button
                  type="button"
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="bg-bg-sidebar text-text-secondary font-medium uppercase text-xs">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-right text-amber-400">Cache</th>
                          <th className="px-4 py-3 text-right text-pink-500">Think</th>
                      ...
                      <tbody className="divide-y divide-border bg-bg-modal">
                        {usage.steps.map((step, index) => (
                          <tr key={index} className="hover:bg-bg-hover transition-colors">
                            <td className="px-4 py-3 font-medium text-text-primary">{step.stepName}</td>
                            <td className="px-4 py-3 text-center">
                              <span className="px-2 py-1 rounded bg-bg-surface border border-border text-xs text-blue-300">
                                {step.tool || "default"}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-right text-text-secondary">{step.input.toLocaleString()}</td>
                            <td className="px-4 py-3 text-right text-amber-400 font-medium">{step.cache.toLocaleString()}</td>
                            <td className="px-4 py-3 text-right text-pink-500 font-medium">{step.think.toLocaleString()}</td>
                            <td className="px-4 py-3 text-right text-text-secondary">{step.output.toLocaleString()}</td>
                            <td className="px-4 py-3 text-right font-bold text-text-primary">
                              {(step.input + step.output + step.think).toLocaleString()}
                            </td>
                          </tr>
                        ))}
                      </tbody>
