...
- **Shared layer**: `PrototypeInputContainer`, `CommandBarBase` e `InputBar` vivem em `src/shared/components`, consumidos via limites de importação controlados pelo script `scripts/check-import-boundaries.mjs`.
...
2. **Shared Layer (fase 3)**  
   - Avaliar se os command bars legados podem ser substituídos definitivamente pelos novos input areas e formalizar o lint de importações dentro do pipeline (ex.: Biome plugin ou rule oficial).
